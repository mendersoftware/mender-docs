...
Add `environment` section if absent. In `environment` section add `DEVICEAUTH_MAX_DEVICES_LIMIT_DEFAULT` variable with integer value. `0` represents `no limit` and is the default.
The updated entry should look like this:
