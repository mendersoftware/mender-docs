...
which specifies what Components are present and how they can be updated. Mender Orchestrator reads the System's desired state from a [Manifest](../03.Manifest/docs.md), which defines the exact combination
...
A Component is a logical software unit that can be updated independently. Components are categorized by Component type and updated through Interfaces.
...
A System can only ever have exactly one Device. In the context of this introduction, the Telematic Control Unit functions
as the System device.

### Update process

Components update sequentially by order value, ascending, as defined in the [Manifest](../02.Manifest/docs.md#update-strategy). Components with the same
order update in parallel (see [ordering](../04.Interface-protocol/docs.md/#ordering)). It is up to a Component Type's Interface to implement rollback.
...
* end state constraints - the acceptable combinations of versions the Components can be once the update completes
* transition state constraints - the acceptable combination of versions the Components can be in while transitioning from one end state to another
...
1. Mender Client recieve an Artifact of payload type `mender-orchestrator-manifest`
...
In order to run Mender Orchestrator with the Mender Client, you must configure the device with the `system` tier.
See [the Installation chapter](../05.Installation/) for steps on how to configure the Device Tier.
...
The regular Device type will be used in all other cases. This allows a System device to recieve and install Manifests
by checking the System type compatibility when needed. It also allows a System device to recieve regular rootfs Updates.
