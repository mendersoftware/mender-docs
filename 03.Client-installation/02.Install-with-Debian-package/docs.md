...
```bash
wget https://downloads.mender.io/3.4.0-build1/dist-packages/debian/$(dpkg --print-architecture)/mender-client_3.4.0-build1-1%2Bdebian%2Bbuster_$(dpkg --print-architecture).deb
```
...
```bash
sudo dpkg -i mender-client_3.4.0-build1-1+debian+buster_$(dpkg --print-architecture).deb
```
...
<!--AUTOVERSION: "mender-client_%-1"/mender -->
```bash
sudo DEBIAN_FRONTEND=noninteractive dpkg -i mender-client_3.4.0-build1-1+debian+buster_$(dpkg --print-architecture).deb
```
...
[README.md](https://github.com/mendersoftware/mender/tree/3.4.0-build1#installing-from-source?target=_blank)
