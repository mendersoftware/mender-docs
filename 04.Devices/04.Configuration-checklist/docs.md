...
1. Before building the images you will test with, run the commands:
...
3. Run the following commands:
   ...
   The reason for doing this manual image patching is to have two slightly different kernels in the image, which will be useful for verification later.

4. Flash the `base-image.sdimg` image to the device and boot it.

5. Verify that the two commands `fw_printenv` and `fw_setenv` are in the path and are executable. Calling them with no arguments will should give a variable listing and an error about missing variable name, respectively.

6. Verify that the booted kernel is from the rootfs. The easiest way to check this is to check the build date, which can be seen by running:

   ```
   cat /proc/version
   ```

   Make sure it matches the build date and time of the first kernel you just built, and not an older one.
...
8. Reboot, and interrupt U-Boot so you get the U-Boot prompt.
...
10. While still at the U-Boot prompt, run:
...
11. You should be back inside Linux now, so run:
...
12. Run the following:
    ...
    If Mender has been enabled as a daemon, either through inheriting `mender-full` or `mender-systemd`, it should return a PID. If not, it should return nothing.

13. Run `mount` with no arguments. The filesystem mounted as root (signified by the `<device> on /` entry) should be `/dev/mmcblk0p2`.
...
16. Repeat step 6, but this time verify that the build date is later than the first time you did this step. This is because we replaced the kernel of the second rootfs partition with a newer one.

17. Repeat step 13, but this time using `/dev/mmcblk0p3`.
...
20. Restore power, boot, and repeat step 13, again with `/dev/mmcblk0p3`. The detected device should *not* be `/dev/mmcblk0p2`, this indicates that the rollback has not worked.
