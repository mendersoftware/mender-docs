...
The Mender-update Client supports multiple ways to customize the update process:
...
The Mender-update Client can run pre- and post-install scripts, before
...
* **Idle**: The Mender-update client idles and waits for the next action to handle. At this stage, no communication with the server or downloads are in progress.
* **Sync**: At this stage, the Mender-update client will either send or update its inventory to the server or check if an update is available. This requires communication with the server.
...
* **ArtifactCommit**: If the new image in the passive partition passes all the integrity checks, the Mender-update client will mark the update as successful and continue running from this partition. The commit makes the update persistent. A pre-script here can add custom integrity checks beyond what the Mender client provides out of the box.
* **ArtifactRollback**: If the new update fails any of the above-mentioned integrity checks, the Mender-update client will revert the update payload. At this point, scripts in this state can help revert migrations done as a part of the update.
...
machine. Depending on the return code from the script, the Mender-update client will:
...
The Mender-update client collects logs during an update. In case of a failure, these
...
!! This feature will be removed in Mender-update client 4.0. For server-side support duration, please refer to our [blog post](https://mender.io/blog/mender-3-6-auto-generation-of-delta-updates#:~:text=Deprecation%3A%20Synchronized%20updates).
