...
Authentication service returns an **authentication token**. The Mender-auth client will record the
token and make it available over a DBus API. Other clients, such as mender-update, will use this
API to fetch the token and attach it to every API call under the HTTP `Authorization` header.

The token does have an **expiry date** (one week period by default), but the Mender-auth client
