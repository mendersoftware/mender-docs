...
|**Query**|**status**  <br>*optional*|Admission status filter. If not specified, all devices are listed.|enum (pending, accepted, rejected)| |
...
|---|---|---|---|
|**Header**|**Authorization**  <br>*required*|Contains the JWT token issued by the User Administration and Authentication Service.|string (Bearer [token])|
|**Path**|**id**  <br>*required*|Device identifier (SHA256 over identity data).|string|
...
|HTTP Code|Description|Schema|
|---|---|---|
|**200**|Successful response - a device is returned.|[Device](#device)|
...
|HTTP Code|Description|Schema|
|---|---|---|
|**200**|The status of the device was successfully updated.|[Status](#status)|
