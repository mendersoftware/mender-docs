...
Now that we have the device's identity and public key, we will use the Mender server management REST APIs to preauthorize it. The APIs are documented in [API chatper](../../200.API/#management-apis).
...
Then simply call the [API to preauthorize a device](../../200.API/#post__devices):
