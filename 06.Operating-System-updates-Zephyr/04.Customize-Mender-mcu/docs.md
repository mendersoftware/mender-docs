...
* `MENDER_SERVER_TENANT_TOKEN` – the authentication token associating the device with your hosted Mender tenant (`CONFIG_MENDER_SERVER_TENANT_TOKEN="<your token here>"`). This must be set for devices connecting to the hosted Mender​. You can obtain the token from your hosted Mender account in the "My organization" menu and compile it into the firmware. Always treat the tenant token as a secret. If you are using an on-premises Mender server (Open Source or Enterprise), you can leave this token empty or unset as it is not used in non-multi-tenant servers.
