...
The `-c` flag for `mender-artifact` was introduced in version 4.4.2. If you are using an older version (e.g. 3.9.0 from the default Ubuntu 24.04/Noble repository), you will see this error when trying to use the `-c` flag.
...
This error occurs when using `mender-artifact` version older than 4.4.2 with Mender 4.0+ devices. The older `mender-artifact` versions are incompatible with the newer device client.

**Solution:**

<!--AUTOVERSION: "version %"/mender-artifact-->
Upgrade your `mender-artifact` to version 4.4.2 or later by following the [installation instructions](../../12.Downloads/01.Workstation-tools/docs.md).
...
The recommended version is 4.4.2 or later for full compatibility with current documentation examples.
