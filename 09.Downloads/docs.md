...
<!-- AUTOMATION: execute=DEBIAN_FRONTEND=noninteractive apt-get install mender-client -->
...
```bash
sudo dpkg -i mender-monitor_master-1+debian+buster_all.deb || apt --fix-broken -y install
```
