...
curl -u $HOSTED_MENDER_EMAIL -O https://downloads.customer.mender.io/content/hosted/mender-binary-delta/1.4.1-build4/mender-binary-delta-1.4.1-build4.tar.xz
```
...
curl -u $MENDER_ENTERPRISE_EMAIL -O https://downloads.customer.mender.io/content/on-prem/mender-binary-delta/1.4.1-build4/mender-binary-delta-1.4.1-build4.tar.xz
```
...
The archive `mender-binary-delta-1.4.1-build4.tar.xz` contains the binaries needed to generate and apply
deltas.
...
<!--AUTOVERSION: "mender-binary-delta-%.tar.xz"/mender-binary-delta-->
Unpack the `mender-binary-delta-1.4.1-build4.tar.xz` in your home directory:

<!--AUTOVERSION: "mender-binary-delta-%.tar.xz"/mender-binary-delta-->
```bash
tar xvf mender-binary-delta-1.4.1-build4.tar.xz
```
...
```
sudo cp mender-binary-delta-1.4.1-build4/x86_64/mender-binary-delta-generator /usr/bin
```
...
<!--AUTOVERSION: "mender-binary-delta-%"/mender-binary-delta-->
```
rm -rf mender-binary-delta-1.4.1-build4
```
...
SRC_URI:pn-mender-binary-delta = "file://${HOME}/mender-binary-delta-1.4.1-build4.tar.gz"
