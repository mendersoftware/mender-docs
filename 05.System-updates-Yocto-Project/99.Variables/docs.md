...
> Value: &lt;mender artifact capsule depends arguments&gt; (default: empty)

Flags added to this variable will be used as extra `depends` arguments to the
`mender-artifact-uefi-capsule` image type when creating the `.mender` artifact.
For example:

```
MENDER_ARTIFACT_CAPSULE_DEPENDS[<guid>] = "2"
```

The above example sets the dependency version for particular &lt;guid&gt; to 2.
This is converted to the following `depends` arguments to the `mender-artifact`
tool.

```
uefi-firmware.<guid>.version:"2"
```

#### `MENDER_ARTIFACT_CAPSULE_NAME`

> Value: &lt;mender artifact capsule name&gt; (default: `MENDER_ARTIFACT_CAPSULE_NAME_DEFAULT`)

The name for the output generated by the `mender-artifact` tool when using
`mender-artifact-uefi-capsule` image type.


#### `MENDER_ARTIFACT_CAPSULE_NAME_DEFAULT`

> Value: &lt;mender artifact capsule default name&gt; (default: `uefi-capsule-<machine name>;`)

The default name for the output generated by the `mender-artifact` tool when
using `mender-artifact-uefi-capsule` image type.


#### `MENDER_ARTIFACT_CAPSULE_NAME_DEPENDS`

> Value: &lt;mender artifact capsule name depends argument&gt; (default: empty)

This defines the `artifact-name-depends` for the output generated by the
`mender-artifact` tool when using `mender-artifact-uefi-capsule` image type.


#### `MENDER_ARTIFACT_CAPSULE_PROVIDES`

> Value: &lt;mender artifact capsule provides arguments&gt;

Flags added to this variable will be used as `provides` arguments to the
`mender-artifact-uefi-capsule` image type when creating the `.mender` artifact.
For example:

```
MENDER_ARTIFACT_CAPSULE_PROVIDES[<guid>] = "tfa:3"
```

The above example sets the `provides` name and version for particular
&lt;guid&gt; to 'tfa' and '3'. This is converted to following `provides`
arguments to the `mender-artifact` tool.

```
uefi-firmware.<guid>.name:"tfa"
uefi-firmware.<guid>.version:"3"
```
